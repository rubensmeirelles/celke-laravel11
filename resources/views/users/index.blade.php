                ...
                <form action="{{ route('user.index') }}">
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-4 col-sm-12">
                            ...
                            <input type="datetime-local" name="start_date_registration" id="start_date_registration"
                                class="form-control" value="{{ $start_date_registration }}">
                        </div>

                        <div class="col-md-4 col-sm-12">
                            <label for="end_date_registration" class="form-label">Data cadastro final</label>
                            <input type="datetime-local" name="end_date_registration" id="end_date_registration"
                                class="form-control" value="{{ $end_date_registration }}">
                        </div>
                        ...
                        <tr>
                            ...
                            <td class="d-md-flex flex-row justify-content-center">
                                <a href="{{ route('user.show', ['user' => $user->id]) }}"
                                    class="btn btn-info btn-sm me-1 mt-1 mt-md-0">Visualizar</a>
                                <a href="{{ route('user.edit', ['user' => $user->id]) }}"
                                    class="btn btn-primary btn-sm me-1 mt-1 mt-md-0">Editar</a>

                                <form action="{{ route('user.destroy', ['user' => $user->id]) }}" method="POST">
                                    @csrf
                                    @method('delete')
                                    <button class="btn btn-danger btn-sm mt-1 mt-md-0" type="submit"
                                        onclick="return confirm('Coonfirma a exclusão do registro?')">Apagar</button>
                                </form>
                            </td>
                        </tr>
                    @empty
